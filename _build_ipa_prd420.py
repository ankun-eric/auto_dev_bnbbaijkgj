#!/usr/bin/env python3
"""[PRD-420] 阶段 4-C: Flutter iOS IPA 打包脚本（GitHub Actions 远程构建 + GH Release）。"""
import os
import sys
import time
import json
...
LOG_FILE = r"C:\auto_output\bnbbaijkgj\_build_ipa_prd420.log"
...
def gen_version_tag():
    ...
    return f"ios-prd420-v{ts}-{suffix}"


def trigger_workflow(version, max_retries=3):
    for i in range(1, max_retries + 1):
        log(f"=== Trigger workflow (attempt {i}/{max_retries}) ===")
        r = run(f'gh workflow run {WORKFLOW} -R {REPO} -f version={version}', timeout=120)
        if r.returncode == 0:
            log("Workflow trigger succeeded")
            return True
        log("Trigger failed, retrying in 10s...")
        time.sleep(10)
    return False
...
def poll_run(run_id, max_wait_sec=2400):
    log(f"=== Polling run {run_id} (max {max_wait_sec}s) ===")
    start = time.time()
    while time.time() - start < max_wait_sec:
        elapsed = int(time.time() - start)
        r = run(f'gh run view {run_id} -R {REPO} --json status,conclusion,url,name', timeout=60)
        if r.returncode == 0 and r.stdout.strip():
            try:
                data = json.loads(r.stdout)
...
def get_failure_logs(run_id):
    ...
    r = run(f'gh run view {run_id} -R {REPO} --log-failed', timeout=120)
    if r.stdout:
...
def main():
    ...
    log("=" * 60)
    log("Stage 4-C: Flutter iOS IPA build via GitHub Actions (PRD-420)")
    log("=" * 60)
    ...
    final = poll_run(run_id, max_wait_sec=2400)
    if final is None:
        result["error"] = f"Build timeout after 40 min. See {actions_url}"
        return finalize(result, start_time)
    ...
    if conclusion != "success":
        ...
        result["error"] = f"conclusion={conclusion}. URL={actions_url}. Log tail:\n{snippet[-2000:]}"
        return finalize(result, start_time)
