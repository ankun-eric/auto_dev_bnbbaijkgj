#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""[PRD-CARE-MODE-OPTIM-V1] 打包小程序 zip 并上传服务器供下载。"""
import os, time, random, zipfile, paramiko
from pathlib import Path

ROOT = Path(__file__).parent
SRC = ROOT / "miniprogram"
HOST = "newbb.test.bangbangvip.com"; USER="ubuntu"; PWD="Newbang888"
PID = "6b099ed3-7175-4a78-91f4-44570c84ed27"
BASE_URL = f"https://newbb.test.bangbangvip.com/autodev/{PID}"
# h5 容器 public 目录可经网关静态访问（已验证 /binni-xiaokang-logo.png 200）
H5 = f"{PID}-h5"

ts = time.strftime("%Y%m%d_%H%M%S"); rnd = "%04x" % random.randint(0, 0xFFFF)
fname = f"miniprogram_care_optim_v1_{ts}_{rnd}.zip"
out = ROOT / fname

print(f"[1/4] 打包 {SRC} -> {fname}")
with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in ("node_modules", ".git", "__pycache__")]
        for f in files:
            if f.endswith(".log"):
                continue
            p = Path(root) / f
            zf.write(p, p.relative_to(SRC.parent))
print(f"   zip 大小: {out.stat().st_size/1024/1024:.2f} MB")

ssh = paramiko.SSHClient(); ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect(HOST, username=USER, password=PWD, timeout=30)
def run(cmd,t=300):
    i,o,e=ssh.exec_command(cmd,timeout=t)
    return o.read().decode("utf-8","ignore"), e.read().decode("utf-8","ignore")

print("[2/4] 上传到服务器并放入 h5 容器 public/downloads")
remote_tmp = f"/tmp/{fname}"
sftp = ssh.open_sftp(); sftp.put(str(out), remote_tmp); sftp.close()
run(f"docker exec {H5} mkdir -p /app/public/downloads")
run(f"docker cp {remote_tmp} {H5}:/app/public/downloads/{fname}")
run(f"rm -f {remote_tmp}")

print("[3/4] 验证下载链接")
url = f"{BASE_URL}/downloads/{fname}"
o,_ = run(f"curl -s -o /dev/null -w '%{{http_code}}' {url}")
print(f"   HTTP={o.strip()}  {url}")

with open(ROOT/"_mp_care_optim_url.txt","w",encoding="utf-8") as f:
    f.write(url)
ssh.close()
print(f"[4/4] DONE -> {url}")
