#!/usr/bin/env python3
"""End-to-end Android APK packaging for h5pay feature.
...
def gen_tag_and_filename():
    ...
    tag = f"android-h5pay-v{ts_full}-{suffix}"
    ts_under = time.strftime("%Y%m%d_%H%M%S")
    fname = f"bini_health_h5pay_{ts_under}_{suffix}.apk"
    return tag, fname
...
def download_apk(tag):
    ...
    apks = [f for f in os.listdir(DOWNLOAD_DIR) if f.endswith(".apk") and tag.replace("android-h5pay-", "h5pay-") in f or tag in f]
    if not apks:
