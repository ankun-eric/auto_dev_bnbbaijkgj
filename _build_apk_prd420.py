# -*- coding: utf-8 -*-
"""[PRD-420] 阶段 4-B：通过 GitHub Actions 远程构建 Flutter Android APK，
...
LOG_PATH = os.path.join(REPO_DIR, "_build_apk_prd420.log")
...
def make_version_tag():
    ...
    return f"android-prd420-v{now}-{h}"
...
def make_remote_filename():
    ...
    h = secrets.token_hex(2)
    return f"app_prd420_{ts}_{h}.apk"
...
def main():
    started = time.time()
    log("=" * 60)
    log("[prd420] starting Android APK remote build pipeline")

    version = make_version_tag()
    log(f"[prd420] version: {version}")
    ...
    log(f"[prd420] run id={run_id} url={run_url}")
    ...
    log(f"[prd420] workflow OK; downloading release {version}")
    tmp_dir = tempfile.mkdtemp(prefix="apk_prd420_")
