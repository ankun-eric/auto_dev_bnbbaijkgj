...
确保 origin/master 拉到最�?commit (5ae72e5) 后再重建容器
"""
import sys
import time
import paramiko
...
def main():
    ...
    log("\n--- 改动确认 ---")
    run(cli, f"grep -c '商菜�? {PROJECT_DIR}/admin-web/src/app/'(admin)'/product-system/orders/dashboard/page.tsx || true")
    run(cli, f"grep -c '_status_code' {PROJECT_DIR}/backend/app/api/merchant_dashboard.py || true")

    log("\n--- 重建 backend (no-cache for app/) ---")
    # backend 通过 COPY . . 复制源码，需要重新构建确�?merchant_dashboard.py 是最新版
