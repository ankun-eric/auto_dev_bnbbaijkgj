#!/usr/bin/env python3
"""[预约日期模式 Bug 修复 v1.0] 增量部署 + 容器内 pytest

策略：
1. SFTP 上传本次改动的后端、admin-web、h5-web 源文件到服务器对应路径
2. 在服务器上 docker compose build + up backend + admin-web + h5-web
3. 在 backend 容器内安装 pytest 并运行新增测试
4. 验证基础访问
"""
import paramiko
import sys
...
FILES = [
    ...
    ("backend/tests/test_checkout_date_mode_no_time_slot.py",
     "backend/tests/test_checkout_date_mode_no_time_slot.py"),
    # H5
...
def main():
    ...
    # 把测试文件 cp 进容器（rebuild 时已 ADD 进去，但保险起见再覆盖一次）
    run(c, f"docker cp {REMOTE_ROOT}/backend/tests/test_checkout_date_mode_no_time_slot.py "
          f"{backend_container}:/app/tests/test_checkout_date_mode_no_time_slot.py")
    ...
    # 运行新增测试
    run(c, f"docker exec -e PYTHONPATH=/app {backend_container} "
          f"python -m pytest tests/test_checkout_date_mode_no_time_slot.py -v --tb=short 2>&1 | tail -80",
        timeout=300)
