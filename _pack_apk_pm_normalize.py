...
def gen_tag_and_filename():
    ...
    tag = f"android-pmnorm-v{ts_full}-{suffix}"
    ts_under = time.strftime("%Y%m%d_%H%M%S")
    fname = f"bini_health_pmnorm_{ts_under}_{suffix}.apk"
    return tag, fname
...
def download_apk(tag):
    ...
    apks = [f for f in os.listdir(DOWNLOAD_DIR) if f.endswith(".apk") and tag.replace("android-pmnorm-", "pmnorm-") in f or tag in f]
    if not apks:
