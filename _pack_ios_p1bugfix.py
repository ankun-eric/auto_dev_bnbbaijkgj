#!/usr/bin/env python3
"""触发 GitHub Actions iOS 构建（payment_method 归一化修复版）并轮询直到完成，最后输出 Release URL 和 IPA 下载 URL."""
import json
import random
import subprocess
import sys
...
def gen_tag():
    ...
    return f"ios-p1bugfix-v{ts}-{suffix}"
...
def main():
    tag = gen_tag()
    print(f"=== iOS PaymentMethod-Normalize IPA Build ===")
    print(f"Tag: {tag}")
    print(f"Repo: {REPO}")
    ...
    ipa_url = ipa.get("url") if ipa else ""
    if not ipa_url and ipa:
        ipa_url = ipa.get("apiUrl") or ""
