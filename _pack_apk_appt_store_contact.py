#!/usr/bin/env python3
"""End-to-end Android APK packaging for the appointment-store contact-row fix.
...
Steps:
  ...
  5. SFTP upload to deploy server static apk dir (sudo cp into nginx-mounted dir).
...
def gen_tag_and_filename():
    ...
    tag = f"android-v{ts_full}-{suffix}"
    ts_under = time.strftime("%Y%m%d_%H%M%S")
    fname = f"bini_health_android_{ts_under}_{suffix}.apk"
    return tag, fname
...
def download_apk(tag):
    ...
    for attempt in range(1, 5):
        ...
        raise RuntimeError(f"gh release download failed after 4 retries: {last_err}")
    apks = [f for f in os.listdir(DOWNLOAD_DIR) if f.endswith(".apk") and tag in f]
    if not apks:
...
def sftp_upload(local_path, remote_filename):
    ...
    try:
        ...
        cmd = (
            f"echo '{SSH_PASS}' | sudo -S mkdir -p {SERVER_APK_DIR} && "
            f"echo '{SSH_PASS}' | sudo -S cp {remote_tmp} {remote_final} && "
            f"echo '{SSH_PASS}' | sudo -S chmod 644 {remote_final} && "
            f"ls -la {remote_final} && "
            f"rm -f {remote_tmp}"
        )
        log(f"  sudo cp -> {remote_final}")
        ...
        log(f"    rc={rc}\n    out={out.strip()}\n    err={err.strip()[:300]}")
        if rc != 0:
            raise RuntimeError(f"server-side cp failed rc={rc} err={err}")
    finally:
        client.close()
    return remote_final
    ...
    for attempt in range(1, 6):
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                if code == 200:
                    return code, int(length) if length and length.isdigit() else None
...
def main():
    ...
    code, remote_size = verify_url(download_url)
    ...
    print(f"HTTP_STATUS    : {code} (verified, remote_size={remote_size})")
    print("OK: 安卓打包成功")
    return 0
