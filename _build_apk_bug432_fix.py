# -*- coding: utf-8 -*-
"""[Bug-432-fix] 通过 GitHub Actions 远程构建 Flutter Android APK，
...
LOG_PATH = os.path.join(REPO_DIR, "_build_apk_bug432fix.log")
...
def make_version_tag():
    ...
    return f"android-bug432fix-v{now}-{h}"
...
def make_remote_filename():
    ...
    h = secrets.token_hex(2)
    return f"app_bug432fix_{ts}_{h}.apk"
...
def main():
    started = time.time()
    log("=" * 60)
    log("[bug432fix] starting Android APK remote build pipeline")

    version = make_version_tag()
    log(f"[bug432fix] version: {version}")
    ...
    log(f"[bug432fix] run id={run_id} url={run_url}")
    ...
    if conclusion != "success":
        ...
        log("=" * 60)
        log("[bug432fix] FAILURE LOG (key 50 lines):")
        lines = tail.splitlines()
        keywords = ("error", "Error", "ERROR", "chat_screen", "chat_message",
                    "Failed", "failed", "FAILURE", "FAILED")
        marked = [i for i, ln in enumerate(lines) if any(k in ln for k in keywords)]
        if marked:
            i0 = max(0, marked[0] - 5)
            i1 = min(len(lines), marked[0] + 50)
    ...
    log(f"[bug432fix] workflow OK; downloading release {version}")
    tmp_dir = tempfile.mkdtemp(prefix="apk_bug432fix_")
