#!/usr/bin/env python3
"""End-to-end Android APK packaging for checkout slot grid (3-column + disabled).
...
def gen_tag_and_filename():
    ...
    tag = f"android-slotgrid-v{ts_full}-{suffix}"
    ts_under = time.strftime("%Y%m%d_%H%M%S")
    fname = f"bini_health_slotgrid_{ts_under}_{suffix}.apk"
    return tag, fname
