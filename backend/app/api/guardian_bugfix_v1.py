...
- 频次防护：5 次/UID/自然日（内存计数器，Redis 不可用时降级）
- 守护人邀请 nickname 必填
- 列表过滤 removed 状态
...
RATE_LIMIT_ACTIONS = ("invite_create", "unguard", "delete_member")
...
def check_and_incr_rate_limit(action: str, uid: int) -> None:
    ...
    with _rate_lock:
        ...
        current = _rate_counter.get(key, 0)
        if current >= RATE_LIMIT_PER_DAY:
            label = {
                "invite_create": "今天已经创建了 5 次邀请啦，明天再继续吧～如有特殊情况请联系客服",
                "unguard": "今天已经解除了 5 次守护啦，明天再继续吧～如有特殊情况请联系客服",
                "delete_member": "今天已经删除了 5 次啦，明天再继续吧～如有特殊情况请联系客服",
            }.get(action, "今日操作次数已达上限，请明天再试")
            raise HTTPException(status_code=429, detail=label)
        _rate_counter[key] = current + 1
...
async def delete_family_member_hard(
    ...
    """
    # 频次防护
    check_and_incr_rate_limit("delete_member", current_user.id)
...
async def unguard_relation(
    ...
    与 DELETE /api/family/management/{id} 行为一致，但增加 5 次/天频次防护。
    """
    check_and_incr_rate_limit("unguard", current_user.id)
