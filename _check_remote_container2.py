...
for cmd in [
    ...
    f"docker exec {CONT} grep -c 'isoformat() + ' /app/tests/test_home_safety_v1.py",
    f"docker exec {CONT} ls -la /app/tests/test_home_safety_v1.py",
    f"docker inspect {CONT} --format '{{{{json .Mounts}}}}'",
    f"cat {BASE}/docker-compose.yml | grep -A 20 'backend:'",
    f"grep -n 'isoformat' {BASE}/backend/app/api/home_safety_v1.py | head -10",
]:
    print("$", cmd)
