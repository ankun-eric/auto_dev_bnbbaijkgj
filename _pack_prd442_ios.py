"""PRD-442 iOS pack: trigger GitHub Actions macOS runner, wait, return Release URLs."""
import json
import os
import secrets
import subprocess
...
LOG_FILE = os.path.join(PROJECT_ROOT, "_pack_prd442_ios.log")
...
def main():
    ...
    log("=== PRD-442 iOS pack START ===")

    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    rand = secrets.token_hex(2)
    tag = f"ios-prd442-v{ts}-{rand}"
