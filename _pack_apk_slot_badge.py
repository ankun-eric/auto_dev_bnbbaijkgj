#!/usr/bin/env python3
"""End-to-end Android APK packaging for checkout slot badge (status badge).
...
def gen_tag_and_filename():
    ...
    tag = f"android-slotbadge-v{ts_full}-{suffix}"
    ts_under = time.strftime("%Y%m%d_%H%M%S")
    fname = f"bini_health_slotbadge_{ts_under}_{suffix}.apk"
    return tag, fname
