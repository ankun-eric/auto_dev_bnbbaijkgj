# -*- coding: utf-8 -*-
"""[PRD-433] 通过 GitHub Actions 远程构建 Flutter Android APK，
...
LOG_PATH = os.path.join(REPO_DIR, "_build_apk_prd433.log")
...
def make_version_tag():
    ...
    return f"android-prd433-v{now}-{h}"
...
def make_remote_filename():
    ...
    h = secrets.token_hex(2)
    return f"app_prd433_{ts}_{h}.apk"
...
def main():
    started = time.time()
    log("=" * 60)
    log("[prd433] starting Android APK remote build pipeline")

    version = make_version_tag()
    log(f"[prd433] version: {version}")
    ...
    log(f"[prd433] run id={run_id} url={run_url}")
    ...
    if conclusion != "success":
        ...
        log("=" * 60)
        log("[prd433] FAILURE LOG (key 50 lines):")
    ...
    log(f"[prd433] workflow OK; downloading release {version}")
    tmp_dir = tempfile.mkdtemp(prefix="apk_prd433_")
