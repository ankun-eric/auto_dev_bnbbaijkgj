 ...
 *   - 引入公共组件 FamilyMemberTabs（数据源切换为 /api/family/members）
 *   - 新增一级 Tab「设备列表 / 报警记录」
 *   - 新增设备备注 remark（必填 ≤20）+ 老设备「编辑备注」弹窗
 */
...
import FamilyMemberTabs, { FamilyMemberLite } from '@/components/family/FamilyMemberTabs';
...
type PrimaryTab = 'devices' | 'alarms';
...
function HomeSafetyPageInner() {
  ...
  const initialPrimaryTab: PrimaryTab = useMemo(() => {
    return searchParams?.get('tab') === 'alarm' ? 'alarms' : 'devices';
  }, [searchParams]);
  ...
  const [bindRemark, setBindRemark] = useState('');
  ...
  const [editRemarkFor, setEditRemarkFor] = useState<BindingItem | null>(null);
  ...
  const isSelfActive = activeMember ? Boolean(activeMember.is_self) : true;
  ...
  const remarkValid = bindRemark.trim().length >= 1 && bindRemark.trim().length <= 20;
  ...
  const doBind = async () => {
    ...
    try {
      ...
      else if (detail.includes('remark_required'))
        showToast('请填写设备备注，方便识别（如：爸爸家）');
      else if (detail.includes('remark_too_long')) showToast('设备备注不超过 20 字');
  ...
  const openEditRemark = (b: BindingItem) => {
    setEditRemarkFor(b);
    setEditRemarkVal(b.remark || '');
  };
  const submitEditRemark = async () => {
    if (!editRemarkFor) return;
    const v = editRemarkVal.trim();
    if (v.length < 1 || v.length > 20) {
      showToast('请填写 1-20 字的备注');
      return;
    }
    try {
      await api.patch(`/api/home_safety/devices/${editRemarkFor.id}/remark`, {
        remark: v,
      });
      showToast('已保存');
      setEditRemarkFor(null);
      loadDevices();
    } catch (e: any) {
      const detail = String(e?.response?.data?.detail || '保存失败');
      if (detail.includes('remark_too_long')) showToast('设备备注不超过 20 字');
      else if (detail.includes('remark_required')) showToast('设备备注不能为空');
      else showToast(detail);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F6F8', paddingBottom: 24 }}>
      ...
      <FamilyMemberTabs
        ...
        onChange={(mid) => setActiveMemberId(mid)}
        onMembersLoaded={(items) => setMembers(items)}
      />
      ...
      >
        {[
          { key: 'devices' as const, label: '设备列表' },
          { key: 'alarms' as const, label: '报警记录' },
        ].map((t) => {
      ...
      {/* 内容区 */}
      {primaryTab === 'alarms' ? (
        <AlarmList memberId={activeMemberId} isSelfActive={isSelfActive} />
      ) : (
        <>
          {/* 迁移提示条（仅本人 Tab） */}
          {showMigrateHint ? (
                ...
                <div style={{ marginTop: 12 }}>
                  {g.items.length === 0 ? (
                    ...
                    g.items.map((b, idx) => {
                      ...
                      const hasRemark = !!(b.remark && b.remark.trim());
                      return (
                        <div
                          key={b.id}
                          style={{
                        ...
                        >
                          ...
                          >
                            <span style={{ color: '#999', fontSize: 12 }}>备注：</span>
                            <span style={{ color: hasRemark ? '#333' : '#BBB', fontWeight: 500 }}>
                              {hasRemark ? b.remark : '未命名'}
                            </span>
                            <button
                              onClick={() => openEditRemark(b)}
                              style={{
                                background: 'transparent',
                                border: 'none',
                                color: '#1F8FE6',
      ...
      {showBindModal ? (
        ...
        >
          ...
          >
            ...
            >
              <span>
                设备备注 <span style={{ color: '#E53935' }}>*</span>
              </span>
              <span style={{ color: '#BBB' }}>{bindRemark.length}/20</span>
            </div>
            <input
              value={bindRemark}
              onChange={(e) => setBindRemark(e.target.value.slice(0, 20))}
              maxLength={20}
              style={{
                width: '100%',
                padding: '10px 12px',
                fontSize: 14,
                border: `1px solid ${remarkValid || bindRemark.length === 0 ? '#ddd' : '#E53935'}`,
                borderRadius: 6,
                marginBottom: 12,
                boxSizing: 'border-box',
              }}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                onClick={doBind}
                disabled={loading || !remarkValid}
                style={{
                  flex: 1,
                  padding: '10px',
                  background: !remarkValid ? '#B3D8F4' : '#1F8FE6',
                  color: '#fff',
                  border: 'none',
                  borderRadius: 6,
                  cursor: !remarkValid || loading ? 'not-allowed' : 'pointer',
                }}
              >
                {loading ? '提交中…' : '确认绑定'}
              </button>
      ...
      {editRemarkFor ? (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>编辑备注</div>
            <div
              style={{
                fontSize: 12,
                color: '#666',
            ...
            >
              <span>设备备注</span>
              <span style={{ color: '#BBB' }}>{editRemarkVal.length}/20</span>
            </div>
            <input
              value={editRemarkVal}
              onChange={(e) => setEditRemarkVal(e.target.value.slice(0, 20))}
              maxLength={20}
              style={{
                width: '100%',
                padding: '10px 12px',
      ...
      {showTransferModal ? (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>调整设备归属</div>
