# 用户体验使用手册 — 七项 Bug 修复 v2（首页 LOGO / 搜索栏 / 我的积分 / AI 咨询）

> 修复版本：bugfix-7 v2  •  发布日期：2026-04-20  •  Commit：`8929203`（修复 Flutter 编译错误后重新打包）

> 上一轮 commit `9a8d544` / `203e3ae` 已完成全部业务修复；本轮在此基础上补修 `flutter_app/lib/services/api_service.dart` 中 `getProductCategories` / `getProducts` 重复定义导致的 APK 构建失败，APK 已通过 GitHub Actions 重新构建并发布到服务器（81.8 MB，2026-04-20 11:47 上传）。
...
| Bug 2 | 首页 / 搜索栏         | H5 / 小程序 / App | 搜索栏 placeholder 显示乱码                     | 全端统一为「搜索您想要的健康服务」，后端启动时自动迁移历史脏数据（幂等）         |
| Bug 3 | 我的积分页 / 顶部卡片 | H5 / 小程序 / App | 「我的积分」深色背景与上方深绿标题颜色冲突      | 卡片背景改为浅绿 `#C8E6C9`，搭配深绿文字；积分数值已对接真实接口（非写死 680）   |
...
>
> 本次最终可达性验证：H5 9/9 全 200，Admin 200，APK 200，小程序 zip 200。
...
### 2.4 安卓 App APK（已重新打包 2026-04-20 11:47）

- 主下载（推荐）：https://newbb.test.bangbangvip.com/autodev/6b099ed3-7175-4a78-91f4-44570c84ed27/apk/bini_health.apk
- 本期固定版本：https://newbb.test.bangbangvip.com/autodev/6b099ed3-7175-4a78-91f4-44570c84ed27/apk/bini_health_bugfix7_android-bugfix7-v20260420-112604-05cd.apk
- 文件大小：约 78 MB（81,815,251 字节）
- 直接用安卓手机浏览器打开任意一个链接下载安装即可（首次安装请允许「未知来源」）。
- 本期 APK 由 GitHub Actions（`.github/workflows/android-build.yml`）在 ubuntu-latest runner 上 Flutter 3.24.0 + Java 17 构建，已修复以前的 `getProductCategories/getProducts` 重复声明问题。

### 2.5 iOS App

- 由于 iOS 必须在 macOS + Xcode 流水线打包，本期未产出 ipa；源码已就绪在 `flutter_app/ios/` 下，可由 iOS 工程师走 macOS 流水线编译。
...
- **预期**：
  ...
  - 完成时间 > 7 天：列表中**不再出现**该任务（前端不需要刷新清单逻辑，由后端 `ONCE_TASK_HIDE_AFTER_DAYS = 7` 自动过滤）
...
### 4.1 业务修复（commit `9a8d544` / `203e3ae`，已部署）
...
| `backend/app/main.py`                               | 启动迁移 `_migrate_v7_search_placeholder()` — 用 `placeholder_v7_normalized` 标志位幂等强制刷新 DB 中可能残留的旧/乱码占位文案         |
...
### 4.2 APK 构建修复（commit `8929203`，本次新增）

| 文件                                       | 变更要点                                                                                                                                |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `flutter_app/lib/services/api_service.dart`| 删除前段（line 153-161）旧版 `getProductCategories` / `getProducts` 重复定义，统一保留后段（line 921+）支持 `keyword` 等参数的新版本   |

> 此前 GitHub Actions APK 构建因为这两个函数被声明两次而 Dart 编译失败 → 本次删除旧定义后重新触发 GH Actions 成功构建并发布到服务器。
...
**Q4：APK 安装后版本号怎么确认？**
A：本期 APK 是 GitHub Release tag `android-bugfix7-v20260420-112604-05cd` 对应的产物，文件大小 81,815,251 字节。如手机安装老 APK，请先卸载再安装新版（包名相同，签名一致可直接覆盖）。

**Q5：APK 下载链接打开很慢/超时怎么办？**
A：APK 已直接发布到测试服务器（与 H5 / API 同一 nginx 域名），不再依赖 GitHub 代理；如仍慢，可尝试切换到企业内网/VPN 后再下载。

---

如有问题请联系开发同学，或在企业微信「客户端测试群」反馈，我们会优先处理。

!#@ 七项Bug修复v2_首页LOGO_搜索栏_我的积分_AI咨询_用户体验手册.md @#!
