#!/usr/bin/env python3
"""Pack miniprogram/ to zip and SFTP upload to project static/miniprogram/ dir.

URL pattern (verified via existing zips):
...
REMOTE_DIR = f"{PROJECT_DIR}/static/miniprogram"
BASE_URL = f"https://{HOST}/autodev/{DEPLOY_ID}/miniprogram"

ts = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
rand = secrets.token_hex(2)
ZIP_NAME = f"miniprogram_reschedule_{ts}_{rand}.zip"
...
def build_zip():
    ...
    if not os.path.isfile(os.path.join(SRC_DIR, "app.json")):
        ...
        sys.exit(2)
    if not os.path.isfile(os.path.join(SRC_DIR, "project.config.json")):
        print("WARN: miniprogram/project.config.json not found (may be required by IDE)")
...
def verify():
    def _do_verify():
        r = requests.get(DOWNLOAD_URL, stream=True, timeout=30, allow_redirects=True)
        size = int(r.headers.get("Content-Length") or 0)
        ctype = r.headers.get("Content-Type", "")
        print(f"[verify] {r.status_code} type={ctype} size={size} url={DOWNLOAD_URL}")
        r.close()
