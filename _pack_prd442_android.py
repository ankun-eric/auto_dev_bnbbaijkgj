#!/usr/bin/env python3
"""
PRD-442 Android APK 打包子 Agent B
...
def gen_version_tag() -> str:
    ...
    return f"android-prd442-v{ts}-{rand_hex}"
...
def rename_apk(src: Path, version_tag: str) -> Path:
    ts = datetime.now().strftime("%Y%m%d%H%M%S")
    rand_hex = "".join(random.choice("0123456789abcdef") for _ in range(4))
    new_name = f"app_prd442_{ts}_{rand_hex}.apk"
...
def main() -> int:
    ...
    try:
        _log("=" * 70)
        _log("PRD-442 Android APK 打包子 Agent B 启动")
        _log("=" * 70)
        ...
        # 4. 下载 APK
        work_dir = PROJECT_ROOT / "_prd442_android_dl"
