"""[2026-05-03 PRD V2 核销订单状态体系优化] 部署 + 容器内 pytest + URL 自检。

- SSH 到 newbb.test.bangbangvip.com (ubuntu / Newbang888)
- 远程项目目录：/home/ubuntu/6b099ed3-7175-4a78-91f4-44570c84ed27/ 或 /home/ubuntu/auto_dev_bnbbaijkgj
- git fetch origin master + reset --hard
- docker compose build backend admin-web h5-web
- docker compose up -d --force-recreate
- 容器内跑 pytest tests/test_orders_status_v2.py
- 关键 URL 自检（/api/health / /api/orders/unified=401 算正常 / /admin / /unified-orders）
"""
...
def main() -> int:
    ...
    try:
        ...
        code, pytest_out, _ = run(ssh,
            f"docker exec {DEPLOY_ID}-backend python -m pytest tests/test_orders_status_v2.py -q 2>&1 | tail -40",
            timeout=600)
        pytest_ok = (code == 0)
        ...
        for path in [
            ...
            "/api/admin/orders/v2/enums",  # 期望 401
            "/api/admin/orders/v2/stats",  # 期望 401
            "/admin/",
            "/unified-orders",             # H5 订单列表
            "/cards",                      # H5 既有功能回归
        ]:
            _, out, _ = run(ssh,
        ...
        # 判定 URL 全部 OK：200/308/401/302 都算正常
