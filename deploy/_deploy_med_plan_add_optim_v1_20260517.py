"""[PRD-MED-PLAN-ADD-OPTIM-V1 2026-05-17] 部署脚本：添加用药计划页面优化。

涉及变更：
- backend/app/api/medication_add_optim_v1.py  新增药品名称联想 API
- backend/app/main.py  注册新 router
- backend/tests/test_med_plan_add_optim_v1_20260517.py  新增 8 项测试
- h5-web/src/components/medication/MedicationFormPanel.tsx  重构表单组件
"""
import sys
import time
...
def main():
    ...
    try:
        ...
        run(
            client,
            f"docker exec {backend_container} sh -c 'cd /app && python -m pytest tests/test_med_plan_add_optim_v1_20260517.py -v --tb=short 2>&1 | tail -80'",
