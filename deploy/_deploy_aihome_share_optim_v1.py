"""[PRD-AIHOME-OPTIM-SHARE-V1 2026-06-02] 部署 AI 首页优化（H5 前端）。

本次仅 H5 前端代码变更（小程序/Flutter 不走 Docker 部署）。
流程：服务器 git fetch+reset 到最新 → 强制无缓存重建 h5-web → 重连 gateway 网络 → 验证。
"""
import sys
sys.path.insert(0, ".")
...
# 2. 生成 BUILD_COMMIT 并强制无缓存重建 h5-web
...
# 5. 重连 gateway 到项目网络（防 down/up 后断网）
