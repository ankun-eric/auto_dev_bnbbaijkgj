# -*- coding: utf-8 -*-
"""[2026-05-02 卡功能 PRD v1.1 第 1 期] 远程部署 + URL 健康检查

复用项目现有目录 /home/ubuntu/{DEPLOY_ID}，使用 docker-compose.prod.yml。
重建 backend / admin-web / h5-web 三个容器。
"""
...
def main() -> int:
    ...
    try:
        ...
        targets = [
            ...
            # 公开接口（C 端卡）
            ("/api/cards?page=1&page_size=5", "api_cards_list"),
            # H5 页面
            ("/h5/cards", "h5_cards_list"),
            ("/h5/cards/wallet", "h5_cards_wallet"),
            # admin 页面
            ("/admin/login", "admin_login"),
            ("/admin/product-system/cards", "admin_cards_page"),
            # 兜底
            ("/h5/login", "h5_login"),
        ]
        fails = []
        ...
        print("\n== 受保护接口冒烟（无 token，期望 401，不应 404/500） ==", flush=True)
        for path, name in [
            ("/api/cards/me/wallet", "cards_me_wallet"),
            ("/api/admin/cards", "admin_cards_list"),
        ]:
            url = f"https://localhost/autodev/{DEPLOY_ID}{path}"
            code, out, _ = run(c, f"curl -sk -o /dev/null -w '%{{http_code}}' '{url}'", timeout=20)
            http = (out.strip() or "000").split()[-1]
