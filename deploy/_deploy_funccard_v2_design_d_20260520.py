"""[PRD-AICHAT-FUNCCARD-V2-DESIGN-D 2026-05-20 v1.2] 方案 D 部署脚本

改动范围（基于 v1.0 在此之上做方案 D · v1.2 升级）：
- 【H5 端】
  - h5-web/src/components/ai-chat/FunctionCardV2.tsx        改造为方案 D 渲染（顶部色条 + 图标左+标题右 + 副标题色块 + 「开始」固定按钮）
  - h5-web/src/components/ai-chat/ChatCards.tsx             调整 buttonToFcv2Data 透传逻辑（buttonText 仅作兼容）
  - h5-web/src/components/ai-chat/QuestionnairePreCard.tsx  buttonText 统一固定「开始」
  - h5-web/src/app/(ai-chat)/ai-home/page.tsx               buttonText 由「开始测评」改为「开始」
- 【admin-web 端】
  - admin-web/src/components/FunctionCardV2Preview.tsx              方案 D 预览复刻
  - admin-web/src/app/(admin)/function-buttons/page.tsx             表单加 v1.2 字段提示（cover_img / button_text 不生效）+ 预览 buttonText=开始
- 【backend tests】
  - backend/tests/test_funccard_v2_20260520.py             升级到 16 个方案 D 校验 TC
...
def main():
    ...
    try:
        ...
        print("\n--- 容器内代码校验（方案 D · v1.2） ---")
        markers = [
            (h5_container, "FunctionCardV2.tsx", "fcv2-top-bar", "H5 V2 顶部色条"),
            (h5_container, "FunctionCardV2.tsx", "fcv2-subtitle-block", "H5 V2 副标题色块"),
            ...
            (admin_container, "FunctionCardV2Preview.tsx", "fcv2-preview-top-bar", "Admin 预览顶部色条"),
            (admin_container, "function-buttons", "fcv2-v12-field-hint-cover", "Admin v1.2 字段提示"),
        ]
        ...
        # 远端 pytest
        print("\n--- backend 容器内 pytest 执行（方案 D · 16 用例） ---")
        rc, out, _ = run(
            client,
            f"docker exec {backend_container} python -m pytest "
            f"tests/test_funccard_v2_20260520.py -v --tb=short --no-header 2>&1 | tail -80",
            ignore_err=True,
            timeout=300,
        )
