"""[HEALTH-PLAN-V1 2026-06-03] 小程序打包 zip + SFTP 上传

输出：miniprogram_health_plan_v1_<timestamp>_<rand>.zip
上传到：服务器静态目录，可通过 BASE_URL/downloads/<file>.zip 浏览器下载
"""
import os
import sys
import zipfile
...
ZIP_NAME = f"miniprogram_health_plan_v1_{TS}_{RAND}.zip"
...
def main():
    ...
    with open(os.path.join(ROOT, "_mp_health_plan_v1_url.txt"), "w", encoding="utf-8") as f:
        f.write(download_url + "\n")
    with open(os.path.join(ROOT, "_mp_health_plan_v1_zipname.txt"), "w", encoding="utf-8") as f:
        f.write(ZIP_NAME + "\n")
