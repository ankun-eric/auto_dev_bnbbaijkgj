...
def main():
    ...
    try:
        ...
        # 4. 重建 h5-web（变更：notifications 页对接真实 API）
        ...
        # 安装测试依赖
        run(ssh,
            f'docker exec {backend_container} pip install --no-cache-dir pytest pytest-asyncio aiosqlite '
            f'-i https://mirrors.cloud.tencent.com/pypi/simple --trusted-host mirrors.cloud.tencent.com --timeout 120 || true',
            timeout=300)
        # 拷贝测试文件
        run(ssh, f'docker cp {PROJECT_DIR}/backend/tests {backend_container}:/app/')
        # 运行新增测试
        run(ssh,
            f'docker exec {backend_container} python -m pytest '
            f'tests/test_order_enhancement.py '
            f'-v --tb=short -p no:cacheprovider 2>&1 | tail -200',
            timeout=600)
        ...
        urls = [
            ...
            f'{BASE_URL}/api/notifications/unread-count',  # 401 期望
            f'{BASE_URL}/api/merchant/business-hours?store_id=1',  # 401 期望
        ]
        for u in urls:
            run(ssh, f'curl -s -k -o /dev/null -w "{u} -> %{{http_code}}\\n" "{u}"', timeout=30)
