"""[PRD-HOME-SAFETY-STATS-INLINE-V1 2026-06-02] 部署 H5 居家安全统计区四项平铺一行改造。
在服务器上 git fetch+reset 到最新，重建 H5 容器（--no-cache），重连 gateway 网络并验证。
"""
import sys
import time
import paramiko
...
step("1. Git fetch + reset", (
    ...
    f"echo '--- grep STATS-INLINE ---' && grep -c 'STATS-INLINE' h5-web/src/app/home-safety/page.tsx"
), timeout=120)
...
# 3. 重建 H5 容器（--no-cache）
code, out, err = step("3. 重建 H5 容器 (--no-cache)", (
