"""[PRD-HEALTH-INFO-SHARED 2026-06-02] 部署「健康档案编辑抽屉化改造 + 公共健康信息子组件」。

本次变更：仅 H5(h5-web) —— 新增 HealthInfoFields 公共组件，health-profile 编辑抽屉与
NewFamilyMemberModal 两处复用。后端无改动。

流程：服务器 git fetch+reset → 强制无缓存重建 h5-web → 重启 → 重连 gateway 网络 → reload。
"""
import sys
sys.path.insert(0, ".")
