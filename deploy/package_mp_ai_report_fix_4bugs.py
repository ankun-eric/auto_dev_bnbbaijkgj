"""AI Report Fix 4 Bugs - package miniprogram zip and upload to server static/downloads.

- Unzipped contents are at the miniprogram root (contains app.json / app.js / project.config.json)
- Unique filename: miniprogram_ai_report_fix_YYYYMMDD_HHMMSS_xxxx.zip
- Also overwrites miniprogram_latest.zip
- After upload, verify download URLs via HTTP HEAD
"""
import os
import sys
import secrets
...
ZIP_NAME = f"miniprogram_ai_report_fix_{ts}_{rand4}.zip"
...
def main():
    ...
    if not os.path.isfile(os.path.join(mp_dir, "app.json")):
        print(f"[FATAL] miniprogram/app.json not found: {mp_dir}")
        sys.exit(2)
    ...
    with zipfile.ZipFile(local_zip, "r") as zf:
        names = zf.namelist()
        assert "app.json" in names, "zip root has no app.json"
    ...
    print(f"  URL_UNIQUE:  {url_unique}")
    print(f"  URL_LATEST:  {url_latest}")
    print(f"  RESULT:      unique={s1}, latest={s2}  -> {'OK' if ok else 'FAIL'}")
    if not ok:
        sys.exit(3)
