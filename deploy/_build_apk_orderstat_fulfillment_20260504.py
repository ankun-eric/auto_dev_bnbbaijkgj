# -*- coding: utf-8 -*-
"""[订单统计 12+7 状态对齐 & 履约方式中文化] 在远程服务器构建最新 APK 并暴露下载链接。"""
import sys
import time
import paramiko
...
def main() -> int:
    ...
    try:
        ...
        cmd = (
            ...
            f"bash -c '{INNER}' 2>&1 | tee /tmp/flutter_build_orderstat.log | tail -80"
        )
        ...
        apk_name = f"bini_health_orderstat_{ts}.apk"
        run(c,
        ...
        with open("/tmp/_apk_url.txt", "w", encoding="utf-8") as f:
            f.write(url + "\n")
        return 0
    finally:
        c.close()
