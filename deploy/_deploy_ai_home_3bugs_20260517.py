"""[BUG_FIX_AI_HOME_3BUGS_20260517] 部署脚本：AI 对话三 Bug 修复（H5 + 后端）。

涉及变更：
- backend/app/utils/ai_output_sanitizer.py：sanitizer 改造（Bug A）
  - 关键词收敛为整句级
  - 清洗粒度从段落级改为行级
  - 取消末尾追加免责声明
- backend/app/api/chat.py：_append_disclaimer 不再追加免责文案
- backend/tests/test_ai_home_3bugs_20260517.py：新增 10 个单测
- h5-web/src/app/(ai-chat)/ai-home/page.tsx
  - handleSelectSession 进历史会话时回填 selectedConsultant（Bug B）
  - loadLastSession 自动恢复时同样回填 selectedConsultant（Bug B）
  - handleNewConversation 新对话强制清空咨询人为本人（Bug C）
"""
import sys
import time
...
def main():
    ...
    try:
        ...
        # 容器内跑新增测试
        print("\n--- Running pytest (sanitizer + drug-identify regression) ---")
        run(
            client,
            f"docker exec {backend_container} sh -c 'cd /app && python -m pytest "
            f"tests/test_ai_home_3bugs_20260517.py "
            f"tests/test_drug_identify_bug_v3_20260516.py "
        ...
        smoke_targets = [
            ("/ai-home", "200/302/308"),
            ("/api/health", "200"),
            ("/api/chat/sessions?limit=1", "200/401"),
        ]
