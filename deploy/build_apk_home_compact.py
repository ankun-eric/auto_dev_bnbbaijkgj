"""首页顶部紧凑化 - 在远程服务器上构建 Flutter APK。

复用 home_3bugs_v3 的成功方案（Docker 内构建 + 国内镜像 + 插件 patch）。
"""
import paramiko
import time
...
def main():
    ...
    # 确认最新代码已拉下来
    run(
        f'grep -n "compact-home-header\\|compact-header\\|AnnotatedRegion" '
        f'{PROJECT_DIR}/flutter_app/lib/screens/home/home_screen.dart '
        f'{PROJECT_DIR}/h5-web/src/app/globals.css '
        f'{PROJECT_DIR}/miniprogram/pages/home/index.wxss 2>&1 | head -20'
    )
    ...
    cmd = (
        ...
        f"docker run --rm --name flutter_home_compact --network=host "
        ...
        f"bash -c '{inner}' 2>&1 | tee /tmp/flutter_build_home_compact.log | tail -100"
    )
    ...
    if 'app-release.apk' not in out:
        ...
        run('tail -200 /tmp/flutter_build_home_compact.log')
        c.close()
        return False
